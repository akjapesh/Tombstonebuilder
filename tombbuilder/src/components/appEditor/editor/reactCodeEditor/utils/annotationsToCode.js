...
export const annotationsToCode = (annotationArray, contentLoaderState) => {
  ...
  annotationArray.forEach((a) => {
    ...
    if (a.type === "rect") {
      ...
      }"  width="${width - (width % SHIFTING_BY_OFFSET)}" height="${height - (height % SHIFTING_BY_OFFSET)}"/> 
      \n`;
      ...
      code += `   
       <circle cx="${cx - (cx % SHIFTING_BY_OFFSET)}" cy="${cy - (cy % SHIFTING_BY_OFFSET)}" r="${
        radius - (radius % SHIFTING_BY_OFFSET)
      }" /> 
      \n
      `;
    }
