  ...
  switch(actionType.target.name){
    ...
    case "speed":
        updateContentLoader(
            ...
            Number(Number(actionType.target.value) <= Number(actionType.target.max) ? (Number(actionType.target.value)<=Number(actionType.target.min)?Number(actionType.target.min):Number(actionType.target.value)) : Number(actionType.target.max))
            );
            break;
