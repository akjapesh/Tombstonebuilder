...
export const centerAlign = (sketchRef) => {
  const calculateCenter = (target) => {
    const center = { centerX: 0, centerY: 0 ,left:0,right:0,top:0,down:0};
    ...
    center.right=shiftValueByOffset(numberFixed(target.left))+shiftValueByOffset(numberFixed(target.width));
    center.top=shiftValueByOffset(numberFixed(target.top));
    center.down=shiftValueByOffset(numberFixed(target.top))+shiftValueByOffset(numberFixed(target.height))
   console.log(center);
    return center;
  };
